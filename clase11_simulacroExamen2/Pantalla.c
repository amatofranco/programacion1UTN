	...
	if (array != NULL && length > 0 && pId != NULL) {
		...
		if (pantalla_emptyIndex(array, length, &indice) == -1) {
			...
			printf("%s", arrayCompleto);
				...
				&& utn_getFloat(&bufferPantalla.precio,
						"Ingrese precio", "Número inválido \n", MIN_PRECIO, MAX_PRECIO,
						2) == 0
...
/* int pantalla_validarId (Pantalla *array, int length) {
	...
	if (array != NULL && length > 0
		...
		if (indice != -1
			...
			case 2:
				...
				if (utn_getAlfanumerica(direccion, "Ingrese la nueva direccion \n",
						"Direccion Inválida \n", 2) == 0) {

					strncpy(array[indice].direccion, direccion, sizeof(direccion));
			...
			case 3:
				...
				if (utn_getFloat(&precio, "Ingrese el nuevo precio\n",
						"Número fuera de rango", MIN_PRECIO, MAX_PRECIO, 2) == 0) {
...
int pantalla_altaForzada(Pantalla* array, int length, int* pId, char* nombre, char* direccion, float precio, int tipo){
	...
	Pantalla bufferPantalla;
		...
		if (array != NULL && length > 0 && pId != NULL &&nombre!=NULL &&direccion!=NULL){
