...
int updateEmployee(Employee *list, int len) {
	...
	if (utn_getNumero(&findId, "Ingrese el Id del empleado a buscar \n",
		...
		if (index != -1 && utn_getNumero(&option,
			...
			case 4:
				...
				if (utn_getNumero(&sector,"Ingrese el nuevo número correspondiente "
					"a sector \n ", "Número inválido \n", 1, QTY_SECTORS, 2) == 0) {
...
int deleteEmployee(Employee *list, int len) {
	...
	if (utn_getNumero(&findId, "Ingrese el Id del empleado a buscar \n",
		...
		if (index == 0 && utn_getNumero(&option, "Seleccione: 1 para borrar - "
			"2 para cancelar operación \n", "Opción inválida \n", 1, 2, 2) == 0
			&& option == 1 && removeEmployee(list, len, findId) == 0) {
		...
		do {
			...
			for (int i = 0; i < len - 1; i++) {
				...
				if (order == 1) {
					...
					if (strncmp(list[i].lastName, list[i + 1].lastName,MAX_NAME) > 0
					|| (strncmp(list[i].lastName, list[i + 1].lastName, MAX_NAME) == 0
