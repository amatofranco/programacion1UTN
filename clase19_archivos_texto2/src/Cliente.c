...
Cliente* cli_newParametros(char *nombre, float altura, char *dni, int id) {
	...
	if (this != NULL && nombre != NULL && altura > 0 && dni != NULL && id > 0) {
...
Cliente* cli_newParametrosTxt(char *nombre, char *altura, char *dni, char *id) {
	...
	if (this != NULL && nombre != NULL && altura != NULL && dni != NULL
			&& dni != NULL) {
			...
			for (int i = 0; i < length; i++) {
				...
				if (arrayPunteros[i] != NULL) {
					...
					if (cli_getNombre(arrayPunteros[i], bufferNombre) == 0
							&& cli_getAltura(arrayPunteros[i], &bufferAltura)
									== 0
							&& cli_getDni(arrayPunteros[i], bufferDni) == 0
							&& cli_getId(arrayPunteros[i], &bufferId) == 0) {

						fprintf(fp, "%s,%.2f,%s,%d\n", bufferNombre,
								bufferAltura, bufferDni, bufferId);
...
int cli_leerArrayEnArchivo(Cliente *arrayPunteros[], int length,
		char *pathArchivo) {
	...
	if (arrayPunteros != NULL && length > 0 && pathArchivo != NULL) {
		...
		if (fp != NULL) {
			...
			do {
				...
				if (fscanf(fp, "%[^,],%[^,],%[^,],%[^\n]\n", bufferNombre,
						bufferAltura, bufferDni, bufferId) == 4) // cant. de variables
						{

					if (cli_agregarAlArrayTxt(arrayPunteros,length,bufferNombre,bufferAltura,bufferDni,bufferId)==0){
...
int cli_agregarAlArrayTxt(Cliente *arrayPunteros[], int length, char *nombre,
		char *altura, char *dni, char *id) {
	...
	if (arrayPunteros != NULL && length> 0&& nombre!= NULL && altura!= NULL && dni!= NULL & id !=NULL) {

		index = cli_getEmptyIndex(arrayPunteros, length);

		if (index >= 0) {

			pCliente = cli_newParametrosTxt(nombre, altura, dni, id);


				if (pCliente != NULL) {

					arrayPunteros[index] = pCliente;
